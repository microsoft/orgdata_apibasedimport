using OrganizationalDataUploadApp;
    ...
    public class Program
    {
        ...
        public void TakeInputs()
        {
            ...
            while (emptyInput)
            {
                ...
                if (appId == string.Empty || pathToFile == string.Empty || tenantId == string.Empty || certName == string.Empty)
                {
        ...
        private async Task RunAsync(string appId, string pathToFile, string tenantId, string certName, IngressDataType ingressDataType)
        {
            ...
            var OrganizationalDataUploadEndPoint = string.Format(
                "{0}/tenants/{1}/modis/connectors/{2}/ingestions/fileIngestion",
                Constants.OrgDataIngestionApi,
                tenantId,
                ingressDataType);
            ...
            try
            {
                HttpResponseMessage message = await client.PostAsync(OrganizationalDataUploadEndPoint, content);
        ...
        private async Task<string> GetAppToken(string tenantId, string appId, string certName)
        {
            ...
            try
            {
                var authResult = await app.AcquireTokenForClient(
                    new[] { $"{Constants.OrgDataIngestionUri}/.default" })
                    .WithSendX5C(true)
                    .ExecuteAsync()
